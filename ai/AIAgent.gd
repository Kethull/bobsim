...
func request_external_action():
	...
	var json_string = JSON.stringify(request_data) # This is already using Godot 4's method
...
func _on_ai_response_received(result: int, response_code: int, _headers: PackedStringArray, body: PackedByteArray):
	if response_code == 200:
		...
		if json_result == null:
			push_error("AI response JSON parsing failed: Invalid JSON format")
			request_builtin_action()  # Fallback
			return
			
		if not json_result is Dictionary:
			push_error("AI response JSON parsing failed: Result is not a Dictionary")
			request_builtin_action()  # Fallback
			return
			
		if not json_result.has("action"):
			push_error("AI response JSON parsing failed: Missing 'action' field")
			request_builtin_action()  # Fallback
			return
		...
		push_error("AI server error: " + str(response_code))
		request_builtin_action()  # Fallback
